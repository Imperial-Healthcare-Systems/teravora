/* eslint-disable */
export function init_solution() {
  if (typeof window === 'undefined') return;
  // Seven solution routes share this runtime. Client-side navigation between two
  // of them remounts SolutionView with fresh DOM, so init must be re-runnable:
  // tear down the previous mount's loops/listeners, then bind to the new nodes.
  if (window.__trv_solution_cleanup) window.__trv_solution_cleanup();
  var cleanups = [];
  window.__trv_solution_cleanup = function () {
    cleanups.forEach(function (f) { try { f(); } catch (e) {} });
    window.__trv_solution_cleanup = null;
  };
var reduce=matchMedia("(prefers-reduced-motion:reduce)").matches;
  // hero headline — staggered word rise-in (per home.runtime.js)
  document.querySelectorAll(".pg-solution .shero .hl-line").forEach(function (line, li) {
  ...
  document.querySelectorAll(".pg-solution .reveal").forEach(function(el){io.observe(el)});
  ...
  document.querySelectorAll(".pg-solution [data-to]").forEach(function(el){if(reduce){el.textContent=el.dataset.to;return}var o=new IntersectionObserver(function(es){es.forEach(function(e){if(e.isIntersecting){count(el);o.unobserve(el)}})},{threshold:.6});o.observe(el);cleanups.push(function(){o.disconnect()})});
  // faq accordion
  document.querySelectorAll(".pg-solution .qa button").forEach(function(b){b.addEventListener("click",function(){b.parentElement.classList.toggle("open")})});
  // hero particles — scattered data resolves near the cursor (lenient gather, per home)
  var cv=document.getElementById("stars");
  if(cv&&!reduce){var ctx=cv.getContext("2d"),W,H,ps=[],mouse={x:-999,y:-999},DPR=Math.min(devicePixelRatio||1,2),rafId,dead=false;
    ...
    function draw(){if(dead)return;ctx.clearRect(0,0,W,H);var active=mouse.x>-500;
    ...
    cleanups.push(function(){dead=true;cancelAnimationFrame(rafId);removeEventListener("resize",sz)})}
  // proof readycard — sequential row reveal + status scramble (per home ledger)
  (function(){var card=document.querySelector(".pg-solution .readycard");if(!card)return;
    ...
    var done=false;var ro=new IntersectionObserver(function(es){es.forEach(function(e){if(e.isIntersecting&&!done){done=true;rows.forEach(function(r,i){setTimeout(function(){r.classList.add("in");if(!reduce){var v=r.querySelector(".v");if(v&&v.dataset.final)scramble(v)}},i*280+120)})}})},{threshold:.4});ro.observe(card);cleanups.push(function(){ro.disconnect()})})();
}
